"""
Mock LLM — không cần API key thật.
"""
import time
import random

MOCK_RESPONSES = {
    "default": [
        "Đây là câu trả lời từ AI agent (mock). Trong production, đây sẽ là response từ OpenAI/Anthropic.",
        "Agent đang hoạt động tốt! (mock response) Hỏi thêm câu hỏi đi nhé.",
        "Tôi là AI agent được deploy lên cloud. Câu hỏi của bạn đã được nhận.",
    ],
    "docker": ["Container là cách đóng gói app để chạy ở mọi nơi. Build once, run anywhere!"],
    "deploy": ["Deployment là quá trình đưa code từ máy bạn lên server để người khác dùng được."],
    "health": ["Agent đang hoạt động bình thường. All systems operational."],
    "antigravity": ["Your code name is Antigravity! A fitting name for a developer reaching new heights."],
    "code name": ["Your code name is Antigravity! I remember you told me earlier."],
}


def ask(question: str, history: list[str] | None = None, delay: float = 0.1) -> str:
    time.sleep(delay + random.uniform(0, 0.05))

    question_lower = question.lower()

    # Check conversation history for context
    if history:
        history_text = " ".join(history).lower()
        if "antigravity" in history_text and ("code name" in question_lower or "what is my" in question_lower):
            return "Your code name is Antigravity! I remember you told me earlier."

    for keyword, responses in MOCK_RESPONSES.items():
        if keyword in question_lower:
            return random.choice(responses)

    return random.choice(MOCK_RESPONSES["default"])


def ask_stream(question: str):
    response = ask(question)
    for word in response.split():
        time.sleep(0.05)
        yield word + " "
