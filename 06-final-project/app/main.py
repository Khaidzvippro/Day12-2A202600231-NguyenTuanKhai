"""
Production AI Agent — Nguyễn Tuấn Khải — Day 12
...
def get_redis() -> redis_lib.Redis | None:
    ...
    if _redis is None:
        try:
            ...
            logger.warning("Redis unavailable — conversation history disabled")
            _redis = None
    return _redis
...
def get_history(user_id: str) -> list[str]:
    ...
    if r is None:
        return []
    try:
        return r.lrange(f"history:{user_id}", 0, 19)  # last 20 messages
    except Exception:
        return []
...
# ─────────────────────────────────────────────────────────
# Rate Limiter — Redis-backed sliding window (shared across workers)
...
def check_rate_limit(key: str):
    ...
    if r is not None:
        # Redis sliding window — shared across all workers
        try:
...
async def lifespan(app: FastAPI):
    ...
    get_redis()  # init redis connection
    time.sleep(0.1)
    _is_ready = True
...
async def ask_agent(
    ...
    # Save to Redis for next turn
    save_to_history(body.user_id, body.question, answer)
